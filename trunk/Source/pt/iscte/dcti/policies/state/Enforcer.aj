...
package pt.iscte.dcti.policies.state;
...
import pt.iscte.dcti.policies.helpers.EclipseConsoleHelper;
import pt.iscte.dcti.policies.helpers.SignatureHandler;
import pt.iscte.dcti.policies.stack.StackReplica;
...
public abstract aspect Enforcer {
	...
	private pointcut exclusions() :
		within(Enforcer+) ||
		within(pt.iscte.dcti.policies.stack.StackReplica+) ||
		within(pt.iscte.dcti.policies.accessibility.classes.Enforcer+) ||
		within(pt.iscte.dcti.policies.accessibility.instances.Enforcer+) ||
		within(pt.iscte.dcti.policies.designbycontract.Enforcer+) ||
		within(pt.iscte.dcti.policies.relationships.association.multiplicity.Enforcer+) ||
		within(pt.iscte.dcti.policies.relationships.association.ordered.Enforcer+) ||
		within(pt.iscte.dcti.policies.relationships.association.unique.Enforcer+);
	...
	// Changer Set
	private pointcut nonStaticStateChangerSet(Object changer, Object to_be_changed) :
		scope() && !exclusions() &&
		setOfNonStaticStateField() &&
		this(changer) && target(to_be_changed);

	// Changer Call
	private pointcut nonStaticStateChangerCall(Object changer, Object to_be_changed) :
		scope() && !exclusions() &&
	...
	// Suspicious Call
	private pointcut nonStaticSuspiciousCall(Object changer, Object to_be_changed) : 
		scope() && !exclusions() &&
		withinCodeOfNonStaticNonStateModifierMethod() && 
		nonStaticStateChangerCall(changer, to_be_changed);

	// Suspicious Set or Call
	private pointcut nonStaticSuspiciousStateChange(Object changer, Object to_be_changed) : 
		scope() && !exclusions() &&
		withinCodeOfNonStaticNonStateModifierMethod() && 
		(nonStaticStateChangerSet(changer, to_be_changed) || nonStaticStateChangerCall(changer, to_be_changed));

	// Illegal State Change (Self)
	private pointcut nonStaticIllegalStateChange(Object changer, Object to_be_changed) :
		scope() && !exclusions() &&
	...
	/*
	 * Capture any set of complex parts, containers of parts (plural multiplicity)
	 */
	private pointcut nonStaticContainerOfPartsArraySet(Object object, Object[] part_array) :
		scope() && !exclusions() &&
	...
	private pointcut nonStaticContainerOfPartsCollectionSet(Object object, Collection part) :
		scope() && !exclusions() && 
	 ...
	 * <p>
	 * 
	 * Captures any normal part set and saves the object part in the aspect's object maps.
	 * 
	 */	
	...
	before(Object object, Object part) : nonStaticNormalPartSet(object, part) {
		...
		if(debug) System.out.println("\tRegistring normal part for " + thisJoinPointStaticPart.getSignature().toLongString() + ".");

		// In the first time a object's part is captured an object's map for parts is created.
		if(!instance_parts.containsKey(object)) {
			parts_shown = false;
			instance_parts.put(object, new HashMap<Signature, Object>());
		}		
		
		// Extracts static part signature from context and saves object's part reference in the map.
		final Signature signature = thisJoinPointStaticPart.getSignature();
		if(!instance_parts.get(object).containsKey(signature) || instance_parts.get(object).get(signature) != part) {
			parts_shown = false;
	 ...
	 * <b>AdviceType:</b> Before
	 * <p>
	 * <b>Scope:</b> Set of object container of parts map, declared as <code>@CollectionOfParts</code>.
	 * <p>
	 * <b>Name:</b> setOfMapContainerOfParts
	 * <p>
	 * 
	 * Captures any map container of parts set and saves it in the aspect's object maps.
	 * 
	 */	
	@SuppressAjWarnings("adviceDidNotMatch")
	before(Object object, Map part_map) : nonStaticContainerOfPartsMapSet(object, part_map) {
		if(debug) System.out.println("(Register) " + thisJoinPointStaticPart + " within " + thisEnclosingJoinPointStaticPart + " {");
		if(debug) System.out.println("\tRegistring part map for " + thisJoinPointStaticPart.getSignature().toLongString() + ".");

		if(!maps_containers_of_instance_parts.containsKey(object)) {
			parts_shown = false;
			maps_containers_of_instance_parts.put(object, new HashMap<Signature, Map>());
		}
		
		final Signature signature = thisJoinPointStaticPart.getSignature();
		
		if(!maps_containers_of_instance_parts.get(object).containsKey(signature) || maps_containers_of_instance_parts.get(object).get(signature) != part_map) {
			parts_shown = false;
			maps_containers_of_instance_parts.get(object).put(signature, part_map);
		}
	 ...
	 * <b>AdviceType:</b> Before
	 * <p>
	 * <b>Scope:</b> Set of object container of parts collection, declared as <code>@CollectionOfParts</code>.
	 * <p>
	 * <b>Name:</b> setOfCollectionContainerOfParts
	 * <p>
	 * 
	 * Captures any collection container of parts set and saves it in the aspect's object maps.
	 * 
	 */	
	@SuppressAjWarnings("adviceDidNotMatch")
	before(Object object, Collection part_collection) : nonStaticContainerOfPartsCollectionSet(object, part_collection) {
		...
		if(!collections_containers_of_instance_parts.containsKey(object)) {
			parts_shown = false;
			collections_containers_of_instance_parts.put(object, new HashMap<Signature, Collection>());
		}
		
		final Signature signature = thisJoinPointStaticPart.getSignature();
		
		if(!collections_containers_of_instance_parts.get(object).containsKey(signature) || collections_containers_of_instance_parts.get(object).get(signature) != part_collection) {
			parts_shown = false;
			collections_containers_of_instance_parts.get(object).put(signature, part_collection);
		}
	 ...
	 * <b>AdviceType:</b> Before
	 * <p>
	 * <b>Scope:</b> Set of object container of parts array, declared as <code>@CollectionOfParts</code>.
	 * <p>
	 * <b>Name:</b> setOfArrayContainerOfParts
	 * <p>
	 * 
	 * Captures any array container of parts set and saves it in the aspect's object maps.
	 * 
	 */	
	@SuppressAjWarnings("adviceDidNotMatch")
	before(Object object, Object[] part_array) : nonStaticContainerOfPartsArraySet(object, part_array) {
		if(debug) System.out.println("(Register) " + thisJoinPointStaticPart + " within " + thisEnclosingJoinPointStaticPart + " {");
		if(debug) System.out.println("\tRegistring part array for " + thisJoinPointStaticPart.getSignature().toLongString() + ".");

		if(!arrays_containers_of_instance_parts.containsKey(object)) {
			parts_shown = false;
			arrays_containers_of_instance_parts.put(object, new HashMap<Signature, Object[]>());
		}
		
		final Signature signature = thisJoinPointStaticPart.getSignature();
		
		if(!arrays_containers_of_instance_parts.get(object).containsKey(signature) || arrays_containers_of_instance_parts.get(object).get(signature) != part_array) {
			parts_shown = false;
			arrays_containers_of_instance_parts.get(object).put(signature, part_array);
		}
	 ...
	 * <p>
	 * 
	 * A limited, though simple, checker. Checks only for illegal self modifications. Redundant.
	 * 
	 */
	@SuppressAjWarnings("adviceDidNotMatch")
	before(Object a, Object b) : nonStaticIllegalStateChange(a, b) {
		throw new IllegalStateModification("\n\t(Self/Shallow) Illegal state modification" +
				" at " + thisJoinPoint.getStaticPart() + " within " + thisEnclosingJoinPointStaticPart +
				"\n\tCaptured at " + EclipseConsoleHelper.convertToHyperlinkFormat(thisJoinPointStaticPart) +
				"\n\twithin " + EclipseConsoleHelper.convertToHyperlinkFormat(thisEnclosingJoinPointStaticPart) + "." + 
				"\n\tNon-@StateModifier origin at " + EclipseConsoleHelper.convertToHyperlinkFormat(thisEnclosingJoinPointStaticPart) + ".");	
	}
	 ...
	 * <p>
	 * 
	 * A full state checker advice. Checks all the object parts and containers of parts.
	 * Checks for, both, shallow and deep modifications.
	 *  
	 */	
	@SuppressAjWarnings("adviceDidNotMatch")
	before(Object possible_part) : nonStaticStateChange(possible_part) {
		...
		verifyNonStaticParts(possible_part, thisJoinPoint, thisEnclosingJoinPointStaticPart);
		verifyStaticPartsForNonStaticExecutions(possible_part, thisJoinPoint, thisEnclosingJoinPointStaticPart);
	}
	...
	before(Object part) : staticNormalPartSet(part) {
		...
		if(!class_parts.get(target_class).containsKey(signature) || class_parts.get(target_class).get(signature) != part) {
			parts_shown = false;
	...
	before(Map part_map) : staticContainerOfPartsMapSet(part_map) {
		...
		if(!maps_containers_of_class_parts.containsKey(target_class)) {
			parts_shown = false;
			maps_containers_of_class_parts.put(target_class, new HashMap<Signature, Map>());
		}
		
		final Signature signature = thisJoinPointStaticPart.getSignature();
		
		if(!maps_containers_of_class_parts.get(target_class).containsKey(signature) || maps_containers_of_class_parts.get(target_class).get(signature) != part_map) {
			parts_shown = false;
			maps_containers_of_class_parts.get(target_class).put(signature, part_map);
		}
	...
	before(Collection part_collection) : staticContainerOfPartsCollectionSet(part_collection) {
		...
		if(!collections_containers_of_class_parts.containsKey(target_class)) {
			parts_shown = false;
			collections_containers_of_class_parts.put(target_class, new HashMap<Signature, Collection>());
		}
		
		final Signature signature = thisJoinPointStaticPart.getSignature();
		
		if(!collections_containers_of_class_parts.get(target_class).containsKey(signature) ||
			collections_containers_of_class_parts.get(target_class).get(signature) != part_collection) {
				parts_shown = false;
				collections_containers_of_class_parts.get(target_class).put(signature, part_collection);
		}
	...
	before(Object[] part_array) : staticContainerOfPartsArraySet(part_array) {
		...
		if(!arrays_containers_of_class_parts.containsKey(target_class)) {
			parts_shown = false;
			arrays_containers_of_class_parts.put(target_class, new HashMap<Signature, Object[]>());
		}
		
		final Signature signature = thisJoinPointStaticPart.getSignature();
		
		if(!arrays_containers_of_class_parts.get(target_class).containsKey(signature) ||
			arrays_containers_of_class_parts.get(target_class).get(signature) != part_array) {
				parts_shown = false;
				arrays_containers_of_class_parts.get(target_class).put(signature, part_array);
		}
	...
	before() : staticIllegalStateChange() {
		Class target_class = thisJoinPointStaticPart.getSignature().getDeclaringType();
		Class this_class = thisEnclosingJoinPointStaticPart.getSignature().getDeclaringType();
	...
	before() : staticStateChange() {
		verifyStaticPartsForStaticExecutions(thisJoinPoint, thisEnclosingJoinPointStaticPart);
	}
	 ...
	 */
	// Definition of a method declared as <code>@PureFunction</code>, outside the scope.
	...
	// Invocation of a method declared as <code>@PureFunction</code>, outside the scope.
	 ...
	 */
	// Definition of a method declared as <code>@StateModifier</code>, outside the scope.
	...
	// Invocation of a method declared as <code>@StateModifier</code>, outside the scope.
	 ...
	 */
	// Get of field declared as <code>@Part</code> outside the scope of the enforcement.
	 ...
	 */
	// Get of field declared as <code>@ContainerOfParts</code> outside the scope of the enforcement.
	 ...
	 */
	// Get of field declared as <code>@NonState</code> outside the scope of the enforcement.
	...
	/*
	 * Set of a field or call to non <code>@PureFunction</code> method "withincode"
	 * of a method declared as <code>@PureFunction</code>.
	 */
	...
	/*
	 * Methods simultaneously declared as <code>@StateModifier</code> and <code>@PureFunction</code>
	 */
	// Call to method simultaneously declared as <code>@StateModifier</code> and <code>@PureFunction</code>.
	...
	// Definition of method simultaneously as <code>@StateModifier</code> and <code>@PureFunction</code>.
	...
	// Set of field simultaneously declared as <code>@NonState</code> and <code>@Part</code>.
	...
	// Set of non-static field declared as @Part being non <code>@InstancePrivate</code>.
	...
	// Get of field simultaneously declared as <code>@NonState</code> and <code>@Part</code>.
	...
	// Get of non-static field declared as @Part being non <code>@InstancePrivate</code>.
	...
	/*
	 * Declarations of fields not eligible to be declared as <code>@ContainerOfParts</code>
	 */
	...
	// Set of field simultaneously declared as <code>@NonState</code> and <code>@ContainerOfParts</code>.
	...
	// Set of field declared as <code>@ContainerOfParts</code> and not being an Array([]), Collection or Map.
	...
	// Set of non-static field declared as @ContainerOfParts being non <code>@InstancePrivate</code>.
	...
	// Get of field simultaneously declared as <code>@NonState</code> and <code>@ContainerOfParts</code>.
	...
	// Get of field declared as <code>@ContainerOfParts</code> and not being an Array([]), Collection or Map.
	...
	// Get of non-static field declared as @ContainerOfParts being non <code>@InstancePrivate</code>.
	 ...
	 */
	// Set of !static !final private field !<code>@InstancePrivate</code>, thus must be declared as <code>@NonState</code>.
	...
	// Set of !static !private field, thus must be declared as <code>@NonState</code>.
	...
	// Get of !static !final private field !<code>@InstancePrivate</code>, thus must be declared as <code>@NonState</code>.
	...
	// Get of !static !private field, thus must be declared as <code>@NonState</code>.
	...
	/**
	 * Verifies if in all of the whole/part relationships exists a part assigned to more than on whole.
	 * 
	...
	private static boolean isPartAssignedToMoreThanOneWhole(Object possible_part)
	{
		...
		for (Object instance : maps_containers_of_instance_parts.keySet())
			for (Map part_map : maps_containers_of_instance_parts.get(instance).values())
		...
		for (Object instance : collections_containers_of_instance_parts.keySet())
			for (Collection part_collection : collections_containers_of_instance_parts.get(instance).values())
		...
		for (Object instance : arrays_containers_of_instance_parts.keySet())
			for (Object[] part_array : arrays_containers_of_instance_parts.get(instance).values())
		...
		for (Class target_class : maps_containers_of_class_parts.keySet())
			for (Map part_map : maps_containers_of_instance_parts.get(target_class).values())
				if (part_map.containsValue(possible_part))
					number_of_part_occurences++;
		
		// Parts in Collection Container of Parts Modification
		for (Class target_class : collections_containers_of_class_parts.keySet())
			for (Collection part_collection : collections_containers_of_instance_parts.get(target_class).values())
				if (part_collection.contains(possible_part))
					number_of_part_occurences++;
				
		// Parts in Array Container of Parts Modification
		for (Class target_class : arrays_containers_of_class_parts.keySet())
			for (Object[] part_array : arrays_containers_of_instance_parts.get(target_class).values())
				for (Object part : part_array)
					if (part == possible_part)
						number_of_part_occurences++;
	 ...
	 * 
	 * Shows the parts of all the objects in the aspect's maps.
	 * 
	 * @pre true
	 * @post true
	 */
	...
	private static void showAllParts() {
		...
		for(Object object : instance_parts.keySet()) {
			...
			for(Signature part_signature : instance_parts.get(object).keySet())
				System.out.println("\t\t\t" + part_signature + " = " + instance_parts.get(object).get(part_signature));
			System.out.println("\t\t}");
		}
		System.out.println("\t}");
	}	
	...
	/**
	 * Returns the <code>JoinPoint</code>s objects of all non static non modifier executions retained in
	 * the <code>StackReplica</code> Aspect.
	 * 
	 * @pre true
	 * @post true
	 * 
	 * @return <code>LinkedList</code>list containing all non modifier executions <code>JoinPoint</code>s.
	 */
	private static LinkedList<JoinPoint> nonStaticNonModifierExecutionsJoinPoints()
	{
		...
		{
			Method executed_method = SignatureHandler.getMethodOf(join_point.getStaticPart().getSignature());
			if(!executed_method.isAnnotationPresent(StateModifier.class) && !Modifier.isStatic(executed_method.getModifiers()))
	...
	/**
	 * Returns the <code>JoinPoint</code>s objects of all static non modifier executions retained in
	 * the <code>StackReplica</code> Aspect.
	 * 
	 * @pre true
	 * @post true
	 ...
	 * @param this_enclosing_join_point_static_part is the JointPoint execution enclosing the JoinPoint.
	 * 
	 */
	private static void verifyNonStaticParts(Object possible_part, JoinPoint this_join_point,
		...
		for (JoinPoint join_point : stack) {
			...
			if (object == possible_part)
				throw new IllegalStateModification("\n\t(Self) Illegal state modification" +
					errorMessageForIllegalStateModifications(join_point, this_join_point, this_enclosing_join_point_static_part));
			...
			if (instance_parts.containsKey(object))
				if (instance_parts.get(object).containsValue(possible_part))
					throw new IllegalStateModification("\n\t(Deep) Illegal state modification" +
						errorMessageForIllegalStateModifications(stack.getFirst(), this_join_point, this_enclosing_join_point_static_part));
				
	
			// Parts in Map Container of Parts Modification
			if (maps_containers_of_instance_parts.containsKey(object))
				for (Map part_map : maps_containers_of_instance_parts.get(object).values())
					if (part_map.containsValue(possible_part))
			...
			// Parts in Collection Container of Parts Modification
			if (collections_containers_of_instance_parts.containsKey(object))
				for (Collection part_collection : collections_containers_of_instance_parts.get(object).values())
					if (part_collection.contains(possible_part))
			...
			// Parts in Array Container of Parts Modification
			if (arrays_containers_of_instance_parts.containsKey(object))
				for (Object[] part_array : arrays_containers_of_instance_parts.get(object).values())
					for (Object part : part_array)
						if (part == possible_part)
	 ...
	 * @param this_enclosing_join_point_static_part is the JointPoint execution enclosing the JoinPoint.
	 */
	private static void verifyStaticPartsForNonStaticExecutions(Object possible_part, JoinPoint this_join_point,
			StaticPart this_enclosing_join_point_static_part)
	{
		LinkedList<JoinPoint> stack = staticNonModifierExecutionsJoinPoints();
				
		for (JoinPoint join_point : stack) {
			Class target_class = join_point.getStaticPart().getSignature().getDeclaringType();
			...
			// Parts in Map Container of Parts Modification
			if (maps_containers_of_class_parts.containsKey(target_class))
				for (Map part_map : maps_containers_of_class_parts.get(target_class).values())
					if (part_map.containsValue(possible_part))
			...
			// Parts in Collection Container of Parts Modification
			if (collections_containers_of_class_parts.containsKey(target_class))
				for (Collection part_collection : collections_containers_of_class_parts.get(target_class).values())
					if (part_collection.contains(possible_part))
			...
			// Parts in Array Container of Parts Modification
			if (arrays_containers_of_class_parts.containsKey(target_class))
				for (Object[] part_array : arrays_containers_of_class_parts.get(target_class).values())
					for (Object part : part_array)
						if (part == possible_part)
	...
	private static void verifyStaticPartsForStaticExecutions(JoinPoint this_join_point,
			StaticPart this_enclosing_join_point_static_part)
	{
		Class target_class = this_join_point.getStaticPart().getSignature().getDeclaringType();
	...
	private static String errorMessageForIllegalStateModifications(JoinPoint join_point,
			JoinPoint this_join_point, StaticPart this_enclosing_join_point_static_part)
	{
		String error_message = " at " + this_join_point.getStaticPart() + " within " + this_enclosing_join_point_static_part +
			"\n\tCaptured at " + EclipseConsoleHelper.convertToHyperlinkFormat(this_join_point.getStaticPart()) +
			"\n\twithin " + EclipseConsoleHelper.convertToHyperlinkFormat(this_enclosing_join_point_static_part) + "." + 
			"\n\tNon-@StateModifier origin at " + EclipseConsoleHelper.convertToHyperlinkFormat(join_point.getStaticPart()) + ".";
	...
	// Non-Static
	private static WeakHashMap<Object, HashMap<Signature, Object>> instance_parts = 
		new WeakHashMap<Object, HashMap<Signature, Object>>();
	private static WeakHashMap<Object, HashMap<Signature, Map>> maps_containers_of_instance_parts = 
		new WeakHashMap<Object, HashMap<Signature, Map>>();
	private static WeakHashMap<Object, HashMap<Signature, Collection>> collections_containers_of_instance_parts = 
		new WeakHashMap<Object, HashMap<Signature, Collection>>();
	private static WeakHashMap<Object, HashMap<Signature, Object[]>> arrays_containers_of_instance_parts = 
		new WeakHashMap<Object, HashMap<Signature, Object[]>>();
	
	
	// Static
	private static WeakHashMap<Class, HashMap<Signature, Object>> class_parts = 
		new WeakHashMap<Class, HashMap<Signature, Object>>();
	private static WeakHashMap<Class, HashMap<Signature, Map>> maps_containers_of_class_parts = 
		new WeakHashMap<Class, HashMap<Signature, Map>>();
	private static WeakHashMap<Class, HashMap<Signature, Collection>> collections_containers_of_class_parts = 
		new WeakHashMap<Class, HashMap<Signature, Collection>>();
	private static WeakHashMap<Class, HashMap<Signature, Object[]>> arrays_containers_of_class_parts = 
		new WeakHashMap<Class, HashMap<Signature, Object[]>>();
