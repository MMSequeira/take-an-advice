...
public abstract aspect Enforcer {
	...
	private pointcut exclusions() :
		...
		within(pt.iscte.dcti.policies.state.Enforcer+);
    ...
    { 
        ...
        if(!instanceMultiplicityIsValidOf(target_object))
            System.out.println(location.getFileName() + ":" + location.getLine() + 
							   " state invalid before " + thisJoinPoint);
    }
		...
		instancesCollectionSet(object, referencesCollection, multiplicity) 
	{
		...
		if(!instances_references_collections.get(object).containsKey(signature) ||
			instances_references_collections.get(object).get(signature).getCollection() != referencesCollection) 
		{
			MultiplicityItem multiplicity_item = new MultiplicityItem(multiplicity.value());
			
			instances_references_collections.get(object).put(signature,
					new ReferencesCollectionData(referencesCollection, multiplicity_item));
		}
		...
		classCollectionSet(referencesCollection, multiplicity)
	{
				...
				class_collections.get(object).get(signature).getCollection() != referencesCollection) 
		{
			...
			class_collections.get(object).put(signature,
					new ReferencesCollectionData(referencesCollection, multiplicity_item));
		}
	...
	/*
	 * Declarations of fields not eligible to be declared with the <code>@Multiplicity</code> qualifier.
	 */
	// Annotate an object, which is not a Collection, with the Multiplicity qualifier
	 ...
	 */
	// Access to a attribute with the @Multiplicity qualifier, outside the scope of the enforcement.
	 ...
	 * 
	 * @return true if the instance's collection multiplicity is valid, false otherwise.
	 */
	private static boolean instanceMultiplicityIsValidOf(Object object) {
    	...
    	if(instances_references_collections.containsKey(object))
			for(ReferencesCollectionData data : instances_references_collections.get(object).values()) {
				if(!data.getMultiplicityItem().contains(data.getCollection().size()))
					return false;
    		}
	...
	/**
	 * Retrieve the instance collection with the invalid multiplicity from the aspect's maps.
	 * 
	 * @pre object != null
	 * @post true
	 * 
	 * @param object is the object containing the instance collection with invalid multiplicity.
	 * 
	 * @return the instance collection with the invalid multiplicity or null if it doesn't exist.
	 */
	...
	/**
	 * Retrieve the class collection with the invalid multiplicity from the aspect's maps.
	 * 
	 * @pre object != null
	 * @post true
	 * 
	 * @param target_class is the class containing the collection with invalid multiplicity.
	 * 
	 ...
	 * Attributes
	 */
	private static WeakHashMap<Object, HashMap<Signature, ReferencesCollectionData>> instances_references_collections =
    	new WeakHashMap<Object, HashMap<Signature, ReferencesCollectionData>>();
	
	private static WeakHashMap<Object, HashMap<Signature, ReferencesCollectionData>> class_collections =
    	new WeakHashMap<Object, HashMap<Signature, ReferencesCollectionData>>();
	...
	/**
	 * Structure containing the reference to a collection captured in the Enforcer and its respective multiplicity.
	 */
    private static class ReferencesCollectionData {
    	/**
    	 * Constructs an object of the ReferencesCollectionData, given a valid collection and its multiplicity.
    	 * 
