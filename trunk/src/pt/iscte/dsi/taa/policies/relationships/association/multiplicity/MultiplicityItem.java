...
public class MultiplicityItem {
	public MultiplicityItem(String value){
		if(!MultiplicityItem.syntaxIsValid(value))
			throw new InvalidMultiplicityValue("Invalid syntax of value.");
	 ...
	 * left_bound := [0-9]+
	 * right_bound := [1-9][0-9]* | '*'
	 * bound := left_bound | left_bound '..' right_bound
	 * multiplicity_sintax := bound (',' bound)* 
	 */
	...
	public String toString()
	{
		...
		while(iterator.hasNext())
			multiplicity_item = multiplicity_item + "," + iterator.next().toString();
