...
public abstract aspect Enforcer {
     ...
     * Collections to save
     */
    private pointcut instancesOrderedCollectionSet(Object object,
    ...
    after(Object target_object): executionOfNonStaticPossibleStateChangerMethodOrConstructor(target_object) {
        ...
        assert instanceCollectionIsOrdered(target_object) :location.getFileName() + ":" + location.getLine() + "\n" +
			"\tordered property invalid after " + thisJoinPoint + "\n" +
			"\tcollection has unordered element at position " + Enforcer.order_verification;
    ...
    before(Object target_object): executionOfNonStaticPossibleStateChangerMethod(target_object) {
        ...
        assert instanceCollectionIsOrdered(target_object) :location.getFileName() + ":" + location.getLine() + "\n" +
			...
			"\tcollection has unordered elements at position " + Enforcer.order_verification;
    ...
    before(Object target_object): executionOfPossibleStateChangerDestructor(target_object) {
        ...
        if (!instanceCollectionIsOrdered(target_object))
            System.out.println(location.getFileName()
                               ...
                               + location.getLine()
                               + "\n"
                               + "\tordered property invalid before "
                               + thisJoinPoint
                               + "\n"
    ...
    before(Object object, Collection<?> orderedCollection) : instancesOrderedCollectionSet(object, orderedCollection) {
        ...
        if (!instances_ordered_collections.get(object).containsKey(signature)
            ...
            instances_ordered_collections.get(object).put(signature,
                                                          orderedCollection);
    ...
    before(Collection<?> orderedCollection) : classOrderedCollectionSet(orderedCollection) {
        ...
        if (!class_ordered_collections.get(object).containsKey(signature)
            ...
            class_ordered_collections.get(object).put(signature,
                                                      orderedCollection);
     ...
     */
    // Access to an attribute with the @Ordered qualifier, outside the scope of
    // the enforcement.
     ...
     */
    // TODO Eliminate warning suppression and attempt to solve generics warnings.
    ...
    private static boolean instanceCollectionIsOrdered(Object object) {
        ...
        if (instances_ordered_collections.containsKey(object))
            for (Collection collection : instances_ordered_collections.get(object)
                                                                      .values()) {
     ...
     * @pre object != null
     * @post true
     * @param target_class is the class whose ordered property is to be
     *            validated.
    ...
    private static boolean classCollectionIsOrdered(Class<?> target_class) {
        ...
        if (class_ordered_collections.containsKey(target_class))
            for (Collection collection : class_ordered_collections.get(target_class)
                                                                  .values()) {
                if (!collection.isEmpty() && collection.size() > 1) {
                    Enforcer.order_verification = 1;
                    @SuppressWarnings("unchecked") Iterator<Comparable> iterator = collection.iterator();
                    Comparable<?> previous_element = iterator.next();
                    while (iterator.hasNext()) {
                        Comparable<Comparable> current_element = iterator.next();
                        if (current_element.compareTo(previous_element) < 0)
    ...
    private static WeakHashMap<Object, HashMap<Signature, Collection<?>>> instances_ordered_collections = new WeakHashMap<Object, HashMap<Signature, Collection<?>>>();

    private static WeakHashMap<Object, HashMap<Signature, Collection<?>>> class_ordered_collections = new WeakHashMap<Object, HashMap<Signature, Collection<?>>>();
